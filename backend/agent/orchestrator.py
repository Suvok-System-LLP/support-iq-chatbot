...
async def run_stream(
    message: str, history: list[dict], user: str = "User"
):
    """Async generator that yields text chunks of the assistant's reply.

    Runs the full agentic tool-call loop (non-streaming) then yields the
    final text in small chunks suitable for SSE delivery.
    ...
    ------
    str
        Small text chunks of the final reply.
    """
    model_name = os.getenv("GEMINI_MODEL", "gemini-1.5-flash")
    client = _build_client()
    ...
    if not text:
        logger.warning("Empty streaming reply from Gemini for user=%s", user)
    ...
    for i, word in enumerate(words):
        ...
        if i < len(words) - 1:
            buf += " "
        if buf.count(" ") >= 3 or i == len(words) - 1:
            yield buf
            buf = ""
            if i < len(words) - 1:
